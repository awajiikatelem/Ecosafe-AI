...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || "https://ecosafe-ai-kovp.onrender.com"}/register`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50/50 relative overflow-hidden px-4">
      ...
      {/* SUCCESS TOAST */}
      {showSuccess && (
        <div className="absolute top-6 bg-green-600 text-white font-semibold text-xs px-6 py-3 rounded-xl shadow-lg z-20 animate-fadeIn">
      ...
      <div className="bg-white w-full max-w-md p-8 sm:p-10 rounded-3xl shadow-xl border border-gray-100 text-center">
        ...
        <p className="text-xs text-gray-400 mt-1.5 mb-8">
          Join EcoSafe Environmental Protection Network
        </p>
